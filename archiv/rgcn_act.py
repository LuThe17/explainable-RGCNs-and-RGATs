...
class RelationalGraphConvolutionNC(Module):
    ...
    def forward(self, x, triples):
        ...
        if x is None:
            ...
            input = self.node_emb#nn.Embedding(num_nodes, in_dim).weight
...
class EmbeddingNodeClassifier(NodeClassifier):
    ...
    def forward(self, x, triples):
        ...
        if input is not None:
            activation['input'] = input.detach()
        out1 = F.relu(out)
        activation['rgcn_no_hidden'] = out1.detach()
        out2, adj, input  = self.rgc1(out1, triples) 
        #out3 = F.relu(out2)
        #activation['rgc1'] = out3.detach()
        #out4 = self.dense(out3)
        out5 = F.softmax(out2, dim=1)
        activation['dense'] = out2.detach()
        return out2, adj, activation
