...
class RGAT(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_relations, num_nodes):
        super().__init__()
    ...
    def forward(self, x, edge_index, edge_type):
        parameter_list = []
        inp, out, w, k, q, K, Q, M, Gmi, Gmj, alpha3, Eij2, exponential, resmat, num_neighbors = self.rgatlayer1(x, edge_index, edge_type)
        out = F.relu(out)
        name = 'l1'
        params = inp, out, w, k, q, K, Q, M, Gmi, Gmj, alpha3, Eij2, exponential, resmat, num_neighbors
        names = 'inp','out', 'w', 'k', 'q', 'K', 'Q',  'M', 'Gmi', 'Gmj', 'alpha3', 'Eij2', 'exponential', 'resmat', 'num_neighbors'
        for par, names in zip(params,names):
            parameter_list.append((f"{names}_{name}", par))
        inp, out, w, k, q, K, Q, M, Gm, alpha, alpha2, Eij, exponential, resmat, num_neighbors = self.rgatlayer2(out, edge_index, edge_type)
        out = F.relu(out)
        name = 'l2'
...
def mepa(self, x, edge_index, edge_type, edge_attr, size, return_attention_weights):
    ...
    E = torch.zeros(self.num_relations,self.num_nodes,self.num_nodes)
    ...
    * If :obj:`attention_mode="multiplicative-self-attention"` and
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.node_emb = Parameter(torch.FloatTensor(num_nodes, in_channels))
        ...
        #self.input = Parameter(torch.Tensor(self.num_nodes, self.in_channels))
        self._alpha = None

        self.reset_parameters()

    def reset_parameters(self):
        #super().__init__().reset_parameters()
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_type: OptTensor = None,
        ...
        """
        inp, out, w, k, q, K, Q, M, Gmi, Gmj, alpha3, Eij2, exponential, resmat, num_neighbors = mepa(self, x, edge_index, edge_type, edge_attr, size, return_attention_weights)
        ...
        # else:
        #print(out)
        return inp, out, w, k, q, K, Q, M, Gmi, Gmj, alpha3, Eij2, exponential, resmat, num_neighbors
